...
function validateExecutionBundle(input) {
  ...
  if (!ensureNumber(input.slippage_bps)) {
    errors.push(validationError('slippage_bps must be number', 'slippage_bps'));
  }
  if (!ensureNumber(input.deadline_seconds)) {
    errors.push(validationError('deadline_seconds must be number', 'deadline_seconds'));
  }
  if (!ensureBoolean(input.prefer_batching)) {
    errors.push(validationError('prefer_batching must be boolean', 'prefer_batching'));
  }
  return errors;
}
...
function handleExecutionBundle(input) {
  ...
  const preferBatching = !!input.prefer_batching;
